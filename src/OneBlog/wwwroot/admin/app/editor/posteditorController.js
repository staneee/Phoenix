angular.module('blogAdmin').controller('PostEditorController', ["$rootScope", "$scope", "$location", "$filter", "$log", "dataService", function ($rootScope, $scope, $location, $filter, $log, dataService) {
    ...
    $scope.load = function () {
        dataService.getItems('/admin/lookups')
        ...
        dataService.getItems(tagsUrl, p)
            .success(function (data) {
                ...
                if ($scope.id) {
                    ...
                    $scope.selectedAuthor = selectedOption($scope.lookups.AuthorList, UserVars.UserId);
                }
            })
            .error(function () {
    ...
    $scope.loadPost = function () {
        ...
        dataService.getItems(url)
            .success(function (data) {
                ...
                $scope.selectedAuthor = selectedOption($scope.lookups.AuthorList, $scope.post.Author.Id);
    ...
    $scope.save = function () {
        ...
        spinOn();
        $scope.post.Author.DisplayName = $scope.selectedAuthor.OptionName;
        $scope.post.Author.Id = $scope.selectedAuthor.OptionValue;
        ...
        if ($scope.post.Id) {
            dataService.updateItem('admin/posts/update', $scope.post)
                .success(function (data) {
                    $scope.refreshPost();
                    $scope.updateCustom();
            ...
            dataService.addItem('admin/posts', $scope.post)
                .success(function (data) {
    ...
    $scope.uploadFile = function (action, files) {
        ...
        dataService.uploadFile("/admin/upload?action=" + action, fd)
            .success(function (data) {
    ...
    $scope.saveCategory = function () {
        ...
        dataService.addItem("/admin/categories", $scope.category)
            .success(function (data) {
    ...
    $scope.saveEditOptions = function () {
        dataService.updateItem('admin/lookups/update', $scope.lookups.PostOptions)
            .success(function (data) {
                toastr.success($rootScope.lbl.postUpdated);
                $("#myModal").modal('hide');
                spinOff();
    ...
    $scope.loadCustom = function () {
        ...
        dataService.getItems('/admin/customfields', { filter: 'CustomType == "POST" && ObjectId == "' + $scope.post.Id + '"' })
            .success(function (data) {
    ...
    $scope.updateCustom = function () {
        ...
        dataService.updateItem("/admin/customfields", $scope.customFields)
            .success(function (data) {
                spinOff();
            })
            .error(function () {
    ...
    $scope.loadProfileCustomFields = function () {
        ...
        dataService.getItems('/admin/customfields', { filter: 'CustomType == "PROFILE"' })
            .success(function (data) {
